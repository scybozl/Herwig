...
namespace Rivet {
  ...
  class ATLAS_NOSHOWER : public Analysis {
  public:

    /// Constructor
    ATLAS_NOSHOWER()
      : Analysis("ATLAS_NOSHOWER"),
...
*/
      ...
      if (elecFS.size() == 1 && muonFS.size() == 1) {
        // 1 positron and 1 negative muon
	...
	unsigned int ithres;
        for (ithres = 0; ithres < threshLimit; ithres++) {
          ...
          // Fill
          if (ihist == 0) _histogram(ihist, ithres)->fill(3, weight); // njets
          //else if (ihist == 1) _histogram(ihist, ithres)->fill(good_jets[0]->pT(), weight); // leading jet pT
          //else if (ihist == 2) _histogram(ihist, ithres)->fill(good_jets[1]->pT(), weight); // 2nd jet pT
          //else if (ihist == 3 && jet_n[ithres] >= 3) _histogram(ihist, ithres)->fill(good_jets[2]->pT(), weight); // 3rd jet pT
          //else if (ihist == 4 && jet_n[ithres] >= 4) _histogram(ihist, ithres)->fill(good_jets[3]->pT(), weight); // 4th jet pT
          //else if (ihist == 5 && jet_n[ithres] >= 5) _histogram(ihist, ithres)->fill(good_jets[4]->pT(), weight); // 5th jet pT
        }
      }
...
private:
  ...
  DECLARE_RIVET_PLUGIN(ATLAS_NOSHOWER);
