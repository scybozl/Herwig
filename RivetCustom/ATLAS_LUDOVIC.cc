...
namespace Rivet {
  ...
  class ATLAS_LUDOVIC : public Analysis {
  public:

    ATLAS_LUDOVIC():
      Analysis("ATLAS_LUDOVIC"),
      _jet_ntag(0),
      _met_et(0.),
      _met_phi(0.),
      _hMap(),
    ...
    void init() {
      ...
      Cut eta_lep = Cuts::abseta < 2.5 && Cuts::pT > 20*GeV;
      // Cut eta_lep = Cuts::abseta < 5.0 && Cuts::pT > 0*GeV;
      ...
      DressedLeptons dressedelectrons(photons, electrons, 0.1, eta_lep, true, true);
      addProjection(dressedelectrons, "dressedelectrons");
      DressedLeptons vetodressedelectrons(photons, electrons, 0.1, eta_lep, true, true);
      addProjection(vetodressedelectrons, "vetodressedelectrons");
      DressedLeptons ewdressedelectrons(photons, electrons, 0.1, eta_lep, true, true);
      ...
      DressedLeptons dressedmuons(photons, muons, 0.1, eta_lep, true, true);
      addProjection(dressedmuons, "dressedmuons");
      DressedLeptons vetodressedmuons(photons, muons, 0.1, eta_lep, true, true);
      addProjection(vetodressedmuons, "vetodressedmuons");
      DressedLeptons ewdressedmuons(photons, muons, 0.1, eta_lep, true, true);
    ...
    void analyze(const Event& event) {
      ...
      _jets = applyProjection<FastJets>(event, "jets").jetsByPt(Cuts::pT > 30*GeV && Cuts::abseta < 2.5);
      // _jets = applyProjection<FastJets>(event, "jets").jetsByPt(Cuts::pT > 0*GeV && Cuts::abseta < 5.0);
...
/*
      ...
      for (i = 0; i < _jets.size(); i++) {
        ...
        // If dR(jet,jet) < 0.4 skip the event
        for (j = 0; j < _jets.size(); j++) {
          const Jet& jet2 = _jets[j];
          if (i == j) continue; // skip the diagonal
          if (deltaR(jet, jet2) < 0.4) _overlap = true;
        }
        // Count the number of b-tags
        if (!jet.bTags().empty() && jet.pT() >= 30*GeV) _jet_ntag += 1;
      }
*/
...
//      if (!pass_ejets || !pass_mujets) vetoEvent;
  ...
  private:
    ...
    bool _ejets(unsigned int& cutBits) {
      ...
      if(charge(_dressedelectrons[0]) <= 0) return false;
      // 4. total neutrino pT > 20 GeV
      cutBits += 1 << 3; if (_met_et <= 20.0*GeV) return false;
      // 5. MTW > 35 GeV
      cutBits += 1 << 4;
    ...
    bool _mujets(unsigned int& cutBits) {
      ...
      if(charge(_dressedmuons[0]) >= 0) return false;
      // 4. total neutrino pT > 20 GeV
      cutBits += 1 << 3; if (_met_et <= 20.0*GeV) return false;
      // 5. MTW > 35 GeV
      cutBits += 1 << 4;
  ...
  DECLARE_RIVET_PLUGIN(ATLAS_LUDOVIC);
